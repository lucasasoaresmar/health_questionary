...
function transformData() {
	...
	//Questão 8
	if (questions.health == 1 && questions.work == 1) {
		score.eigth = 6;
	} else if (questions.health > 1) {
	...
	dor: {point: score.seventh + score.eigth, min: 5, var: 20},
	geral: {point: score.first + score.eleventh, min: 4, var: 20},
...
function rawScale () {
	Object.getOwnPropertyNames(domain).forEach(function(name) {
		console.log((domain[name].point - domain[name].min)*100/domain[name].var);
		});
}
